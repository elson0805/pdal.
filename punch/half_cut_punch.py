...
def PunchMaking():
    ...
    total_op_number = 7
    plate_line_half_cut_line_number = [[0] * 99 for i in range(99)]
    plate_line_half_cut_line_number[1][2] = 1
    plate_line_half_cut_line_number[1][3] = 0
    plate_line_half_cut_line_number[1][4] = 0
    plate_line_half_cut_line_number[1][6] = 0
    plate_line_half_cut_line_number[1][7] = 0

    for now_op_number in range(1, total_op_number + 1):
        n = now_op_number
        op_number = 10 * n
        if plate_line_half_cut_line_number[g][n] > 0:
            for now_data_number in range(1, plate_line_half_cut_line_number[g][n] + 1):  #
                part_open('Data1.CATPart')
                interferance_pad_name = "_half_cut_punch_"
                interferance_line_name = "_half_cut_line_"
                open_file_name = "QR_half_cut_punch"
        ...
        #     for now_data_number in range(1, plate_line_A_punch_number[g][n] + 1):
        #         part_open('Data1.CATPart')
        #         interferance_pad_name = ""
        #         interferance_line_name = ""
        #         open_file_name = ""
        #         punch(open_file_name)
        #         punch_change(interferance_pad_name, interferance_line_name, now_plate_line_number, op_number,
        #                      now_data_number, total_op_number)
        ...
        #         part_open('Data1.CATPart')
        #         interferance_pad_name = "_Bending_punch_"
        #         interferance_line_name = "_forming_cavity_"
        #         open_file_name = "Bending_punch_1"
        #         punch(open_file_name)
        #         Bending_punch_change(interferance_pad_name, interferance_line_name, now_plate_line_number, op_number,
        #                      now_data_number, total_op_number)
...
def punch_change(InterferancePadName, InterferanceLineName, NowPlateLineNumber, OpNumber, NowDataNumber,
    ...
    length0 = parameter1.Item("QR_half_punch_up_plane")
    if Mode_status == '開模':
        length0.value = (float(strip_parameter_list[1]) + float(strip_parameter_list[20]) + float(
            strip_parameter_list[17]) + float(strip_parameter_list[14]) + 28)
    elif Mode_status == '閉模':
        length0.value = (float(strip_parameter_list[1]) + float(strip_parameter_list[20]) + float(
            strip_parameter_list[17]) + float(strip_parameter_list[14]))
    length1 = parameter1.Item('QR_half_punch_height')
    cut_punch_height = 0
    if cut_punch_height == None:
        length1.value = QR_punch_height
    else:
    ...
    # ========設定沖頭==========
    length2 = parameter1.Item("QR_half_punch_Hanging_Desk_height")
    length2.value = float(strip_parameter_list[14])
    strParam1 = parameter1.Item("TYPE")
    strParam1.Value = NowDataNumber
    # ========草圖置換==========
