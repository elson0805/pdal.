...
# 檔案路徑
file_path = ('C:\\Users\\PDAL\\Desktop\\VB-GTCA022')
# 儲存路徑 (output 零件)
...
def PunchMaking():
    ...
    total_op_number = 7
    plate_line_forming_cavity_surface_number = [[0] * 99 for i in range(99)]
    plate_line_forming_cavity_surface_number[1][2] = 1
    plate_line_forming_cavity_surface_number[1][3] = 0
    plate_line_forming_cavity_surface_number[1][4] = 0
    plate_line_forming_cavity_surface_number[1][6] = 0
    plate_line_forming_cavity_surface_number[1][7] = 0

    for now_op_number in range(1, total_op_number + 1):
        n = now_op_number
        op_number = 10 * n
        if plate_line_forming_cavity_surface_number[g][n] > 0:
            for now_data_number in range(1, plate_line_forming_cavity_surface_number[g][n] + 1):
                part_open('Data1.CATPart')
                interferance_pad_name = "_forming_cavity_"
                interferance_line_name = "_forming_cavity_Project_"
                open_file_name = "forming_punch"
...
def punch_change(InterferancePadName, InterferanceLineName, NowPlateLineNumber, OpNumber, NowDataNumber,
    ...
    # length0 = parameter1.Item("forming_punch_center_point_distance_X")
    # length0.Value = ((OpNumber / 10) - 1) * pitch
    die_open_height = str(0)
    length1 = parameter1.Item("forming_punch_up_plane")
    length1.Value = die_open_height
    length2 = parameter1.Item('forming_punch_height')
    g = NowPlateLineNumber
    length2.value = bb(str(NowPlateLineNumber))
    part1.Update()
    # ========設定沖頭==========
    ...
    upper_die_open_height = 28
    hybridShapeFactory1 = part1.hybridShapeFactory
    hybridShapeSurfaceExplicit1 = parameter1.Item(
        "die\\plate_line_" + str(NowPlateLineNumber) + "_op" + str(OpNumber) + "_forming_cavity_surface_" + str(
            NowDataNumber))
    ...
    body1 = bodies1.Item("Body.2")  # 設定工作物件
    ...
    part1.InWorkObject.Name = "plate_line_" + str(NowPlateLineNumber) + "_op" + str(
        OpNumber) + "_forming_cavity_Boundary_" + str(NowDataNumber)  # 更改外型線名稱
    ...
    # ========Project投影外形線至指定平面========
    hybridShapeCurveExplicit1 = parameter1.Item(
        "plate_line_" + str(NowPlateLineNumber) + "_op" + str(
            OpNumber) + "_forming_cavity_Boundary_" + str(NowDataNumber))  # 欲投影外形線之名稱
    reference1 = part1.CreateReferenceFromObject(hybridShapeCurveExplicit1)
    # body1 = bodies1.Item("PartBody")
    hybridShapes1 = body1.HybridShapes
    hybridShapePlaneOffset1 = hybridShapes1.Item("forming_punch_down_plane")  # 投影到哪個平面
    ...
    part1.InWorkObject.Name = "plate_line_" + str(NowPlateLineNumber) + "_op" + str(
        OpNumber) + "_forming_cavity_Project_" + str(NowDataNumber)  # 更改外形線名稱
    hybridShapeFactory1.DeleteObjectForDatum(reference3)

    # ========草圖置換==========
    cut_line_formula_1_name = (str(
        "`die\\plate_line_" + str(NowPlateLineNumber) + "_op" + str(OpNumber) + str(interferance_line_name) + str(
            NowDataNumber)))
    ...
    hybridShapeCurveExplicit1 = parameter1.Item(
        "plate_line_" + str(NowPlateLineNumber) + "_op" + str(OpNumber) + str(interferance_line_name) + str(
            NowDataNumber))  # 草圖
    reference1 = part1.CreateReferenceFromObject(hybridShapeCurveExplicit1)
    pad1 = shapeFactory1.AddNewPadFromRef(reference1, 40)
    limit1 = pad1.FirstLimit
    limit1.LimitMode = catUpToSurfaceLimit
    hybridShapeSurfaceExplicit1 = parameter1.Item(
        "die\\plate_line_" + str(NowPlateLineNumber) + "_op" + str(OpNumber) + "_forming_cavity_surface_" + str(
            NowDataNumber))  # 長到哪個面
    reference2 = part1.CreateReferenceFromObject(hybridShapeSurfaceExplicit1)
