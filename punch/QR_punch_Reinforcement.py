...
def PunchMaking():  # 補強沖頭
    now_plate_line_number = 1
    g = now_plate_line_number
    total_op_number = 7
    plate_line_Reinforcement_cut_line_number = [[0] * 99 for i in range(99)]
    plate_line_Reinforcement_cut_line_number[1][2] = 1
    plate_line_Reinforcement_cut_line_number[1][3] = 0
    plate_line_Reinforcement_cut_line_number[1][4] = 0
    plate_line_Reinforcement_cut_line_number[1][6] = 0
    plate_line_Reinforcement_cut_line_number[1][7] = 0

    for now_op_number in range(1, total_op_number + 1):
        n = now_op_number
        op_number = 10 * n
        if plate_line_Reinforcement_cut_line_number[g][n] > 0:
            for now_data_number in range(1, plate_line_Reinforcement_cut_line_number[g][n] + 1):  #
                part_open('Data1.CATPart')
                interferance_pad_name = "_QR_punch_"
                interferance_line_name = "_Reinforcement_cut_line_"
                open_file_name = "QR_punch_Reinforcement"
...
def punch_change(InterferancePadName, InterferanceLineName, NowPlateLineNumber, OpNumber, NowDataNumber,
    ...
    length1 = parameter1.Item("QR_punch_up_plane")
    if Mode_status == '開模':
    ...
    length2 = parameter1.Item('QR_punch_height')
    cut_punch_height = 0
    if cut_punch_height == None:
        length2.value = cut_punch_height
    else:
    ...
    strParam1 = parameter1.Item("QR_type")
    strParam1.Value = "QR_B"
    length1 = part1.Parameters.Item("QR_wide")
    length2 = part1.Parameters.Item("Reinforcement_Grow_wide_2")
    if length1.Value < 10:
        length2.Value = 10 - length1.value
        strParam1.Value = "Reinforcement_C"
        mean = True
    else:
        strParam1.Value = "QR_B"
        mean = False
    length0 = part1.Parameters.Item("Reinforcement_Grow_wide_1")
    length0.Value = 20
    length0 = part1.Parameters.Item("Reinforcement_Grow_long_1")
    length0.Value = 46
    length0 = part1.Parameters.Item("Reinforcement_Grow_long_2")
    length0.Value = 25
    length0 = part1.Parameters.Item("Bolt_long")
    if mean == False:
        length0.Value = 9
    else:
        length0.Value = 9 + length2.Value
    part1.Update()
    # ========改變參數==========
