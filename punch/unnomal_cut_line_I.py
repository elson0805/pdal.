...
def PunchMaking():
    ...
    total_op_number = 7
    plate_line_unnomal_cut_line_I_number = [[0] * 99 for i in range(99)]
    plate_line_unnomal_cut_line_I_number[1][2] = 1
    plate_line_unnomal_cut_line_I_number[1][3] = 0
    plate_line_unnomal_cut_line_I_number[1][4] = 0
    plate_line_unnomal_cut_line_I_number[1][6] = 0
    plate_line_unnomal_cut_line_I_number[1][7] = 0

    for now_op_number in range(1, total_op_number + 1):
        n = now_op_number
        op_number = 10 * n
        if plate_line_unnomal_cut_line_I_number[g][n] > 0:
            for now_data_number in range(1, plate_line_unnomal_cut_line_I_number[g][n] + 1):  #
                part_open('Data1.CATPart')
                interferance_pad_name = "_unnomal_cut_punch_I_"
                interferance_line_name = "_unnomal_cut_line_I_"
                open_file_name = "unnomal_cut_punch_I"
...
def punch_change(InterferancePadName, InterferanceLineName, NowPlateLineNumber, OpNumber, NowDataNumber,
    ...
    line2_name = str(
        "die\\plate_line_" + str(NowPlateLineNumber) + "_op" + str(OpNumber) + "_unnomal_cut_line_I_symmetric_" + str(
            NowDataNumber))
    line3_name = str("die\\plate_line_" + str(NowPlateLineNumber) + "_op" + str(OpNumber) + str(InterferanceLineName) + str(
        NowDataNumber))
    ...
    # ========草圖置換==========
    parameters1 = parameter1.Item("1_UUU_1")
