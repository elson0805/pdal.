...
def PunchMaking():
    ...
    total_op_number = 7
    plate_line_emboss_forming_punch_right_surface_number = [[0] * 99 for i in range(99)]
    plate_line_emboss_forming_punch_right_surface_number[1][2] = 1
    plate_line_emboss_forming_punch_right_surface_number[1][3] = 0
    plate_line_emboss_forming_punch_right_surface_number[1][4] = 0
    plate_line_emboss_forming_punch_right_surface_number[1][6] = 0
    plate_line_emboss_forming_punch_right_surface_number[1][7] = 0

    for now_op_number in range(1, total_op_number + 1):
        n = now_op_number
        op_number = 10 * n
        if plate_line_emboss_forming_punch_right_surface_number[g][n] > 0:
            for now_data_number in range(1, plate_line_emboss_forming_punch_right_surface_number[g][n] + 1):  #
                part_open('Data1.CATPart')
                interferance_pad_name = "_emboss_forming_punch_right_"
                interferance_line_name = "_emboss_forming_punch_right_surface_"
                open_file_name = "emboss_forming_punch"
...
def punch_change(InterferancePadName, InterferanceLineName, NowPlateLineNumber, OpNumber, NowDataNumber,
    ...
    length1 = parameter1.Item("punch_down_plane_height")
    if Mode_status == '開模':
        upper_die_open_height = 28
        length1.value = (float(upper_die_open_height))
    elif Mode_status == '閉模':
        length1.value = (float(strip_parameter_list[1]))
    length2 = parameter1.Item('punch_up_plane_height')
    punch_up_plane_height = None
    if punch_up_plane_height != None:
        length2.value = punch_up_plane_height
    else:
        length2.value = (
                    float(strip_parameter_list[20]) + float(strip_parameter_list[17]) + float(strip_parameter_list[14]))
    length3 = parameter1.Item("hang_plane_height")
    length3.value = float(strip_parameter_list[14])
    # length4 = parameter1.Item("gap")
    # length4.value = 0
    length5 = parameter1.Item("hang_direction")
    length5.value = "right"
    # ========設定沖頭==========

    # ========草圖置換==========
    part1.Update()
    forming_surface_assume_1_name = (str(
        "`die\\plate_line_" + str(NowPlateLineNumber) + "_op" + str(OpNumber) + str(InterferanceLineName) + str(
                NowDataNumber) + '`'))
    parameters1 = parameter1.Item("forming_surface_formula_1")
    formula1 = parameters1.OptionalRelation
    # relaitons = part1.Relations
    # formula1 = relaitons.Item("forming_surface_formula_1")
    formula1.Modify(forming_surface_assume_1_name)
    formula1.Rename(forming_surface_assume_1_name)
    part1.Update()
    # ========草圖置換==========
