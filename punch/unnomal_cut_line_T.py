...
def PunchMaking():
    ...
    total_op_number = 7
    plate_line_unnomal_cut_line_T_number = [[0] * 99 for i in range(99)]
    plate_line_unnomal_cut_line_T_number[1][2] = 1
    plate_line_unnomal_cut_line_T_number[1][3] = 0
    plate_line_unnomal_cut_line_T_number[1][4] = 0
    plate_line_unnomal_cut_line_T_number[1][6] = 0
    plate_line_unnomal_cut_line_T_number[1][7] = 0

    for now_op_number in range(1, total_op_number + 1):
        n = now_op_number
        op_number = 10 * n
        if plate_line_unnomal_cut_line_T_number[g][n] > 0:
            for now_data_number in range(1, plate_line_unnomal_cut_line_T_number[g][n] + 1):  #
                part_open('Data1.CATPart')
                interferance_pad_name = "_unnomal_cut_punch_T_"
                interferance_line_name = "_unnomal_cut_line_T_"
                open_file_name = "unnomal_cut_punch_T"
...
def punch_change(InterferancePadName, InterferanceLineName, NowPlateLineNumber, OpNumber, NowDataNumber,
    ...
    now_unnomal_cut_line_T_number = 2
    line2_name = ("die\\plate_line_" + str(NowPlateLineNumber) + "_op" + str(
        OpNumber) + "_unnomal_cut_line_T_symmetric_" + str(
        NowDataNumber))
    line3_name = (
    ...
    selection1.clear()
    selection1.Search("Name=" + line2_name)
    if selection1.Count > 0:
        line1_name = line2_name
    else:
        line1_name = line3_name
    cut_line_st = str(line1_name)
    # ========設定沖頭==========
    splint_height = 20
    stop_plate_height = 16
    stripper_plate_height = 16
    length1 = parameter1.Item("T_punch_length")
    length1.Value = splint_height + stop_plate_height + stripper_plate_height + 5
    length2 = parameter1.Item("T_down_thickness")
    length2.Value = -5
    part1.Update()

    # ========草圖置換==========
    parameters1 = parameter1.Item("1_TTT_1")
